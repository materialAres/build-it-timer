...
Questa sezione definisce le regole vincolanti per tutto il codice scritto nel progetto, a prescindere da chi (o quale IA) lo scriva. Ogni task nella Sezione 2 deve rispettarle: sono un prerequisito implicito di ogni criterio di accettazione. L’obiettivo è mantenere l’estensione:
...
- priva di logica di business sparsa tra UI, background e content script.
...
| **KISS** | Il primo motore di crescita città usa una griglia 2D semplice con stati enum per cella (`empty`, `growing`, `building`, `destroyed`), non un sistema fisico o a particelle. L'ottimizzazione con TexturePacker/sprite sheet è esplicitamente rimandata (vedi Milestone 4) e non deve complicare l'MVP. |
...
│   ├── city/
│   │   ├── growth-engine.ts     # calcolo stato griglia da tempo trascorso
│   │   └── palette.ts           # hash dominio → colore deterministico
│   ├── score/
│   │   └── calculate-score.ts   # Excellent/Good/Bad + malus
│   └── timer/
│       └── alarm-adapter.ts     # interfaccia + implementazione browser.alarms
├── assets/
│   └── ascii/                   # eventuali frame/template ASCII statici
...
* **Store persistito** (middleware `persist`, backend `browser.storage.local` tramite adapter custom): tutto ciò che deve sopravvivere al riavvio del browser e rappresenta dati "storici" o configurazione dell'utente —
  * stato della città (griglia, edifici, palette assegnate ai domini);
  * configurazione allowlist/blocklist, preset personalizzati e relativi tag;
  * storico sessioni di focus (per lo score aggregato, se richiesto in futuro) — *vedi Punti da chiarire, §4*;
...
* **Store volatile** (non persistito, reset a ogni avvio del service worker/popup): tutto ciò che è derivato o transitorio —
  * timer "in corso" a grana fine (i secondi correnti non ancora arrotondati al tick di 60s — va comunque riconciliato con `browser.alarms`, che è la fonte di verità per la persistenza reale del countdown, vedi §3.4);
...
| M2.T7 | Applicazione regole DNR dal background | 2 | M2.T6, M1.T7 |
| M2.T8 | Content script overlay alert (Shadow DOM) | 2 | M1.T6, M2.T4 |
| M2.T9 | `scoreSlice` + `lib/score/calculate-score.ts` | 2 | M1.T4 |
| M2.T10 | Calcolo malus su accesso a sito bloccato | 2 | M2.T9, M2.T8 |
| M2.T11 | `citySlice` — stato griglia città | 2 | M1.T4 |
| M2.T12 | `lib/city/growth-engine.ts` | 2 | M2.T11, M2.T1 |
| M2.T13 | `lib/city/palette.ts` (hash dominio → colore) | 2 | M1.T2 |
| M2.T14 | `CityCanvas` + `CityCell` (render ASCII + glow CRT) | 2 | M2.T12, M2.T13 |
| M2.T15 | Collegamento growth-engine ↔ timer tick | 2 | M2.T12, M2.T2 |
| M2.T16 | Collegamento malus ↔ distruzione edifici | 2 | M2.T10, M2.T12 |
...
| M3.T3 | `SiteList` — CRUD siti in allow/blocklist | 3 | M2.T4, M1.T2 |
| M3.T4 | Popup `App.tsx` — composizione UI completa | 3 | M2.T3, M2.T14, M2.T17, M3.T1–M3.T3 |
| M3.T5 | Animazioni `framer-motion` su crescita/distruzione edifici | 3 | M2.T14 |
| M3.T6 | Persistenza multi-browser: verifica Firefox/Chrome build | 3 | M3.T4 |
| M3.T7 | Layout mobile-first popup | 3 | M3.T4 |
...
| M4.T4 | Rifinitura accessibilità popup | 4 | M3.T7 |
...
* **Obiettivo**: definire in `lib/`/`store/` i tipi TypeScript condivisi: `Domain`, `BlocklistEntry`, `Preset`, `Tag`, `CityCell`, `CityGrid`, `ScoreLevel`, `TimerState`, `RuntimeMessage` (union discriminata).
* **File**: `store/store.types.ts`, `components/city/city.types.ts`, un file `lib/messages.types.ts` per `RuntimeMessage`.
* **Dipendenze**: M0.T5
* **Criteri di accettazione**:
  - Nessun tipo usa `any`.
  - `RuntimeMessage` è una union discriminata su campo `type`, con almeno le varianti: `TIMER_TICK`, `TIMER_STARTED`, `TIMER_PAUSED`, `SITE_BLOCKED_ATTEMPT`, `MALUS_APPLIED`.
...
* **Obiettivo**: slice con stato `{ status: 'idle'|'running'|'paused', remainingSeconds, sessionStartedAt }` e azioni `startTimer`, `pauseTimer`, `resetTimer`.
...
* **Criteri di accettazione**:
  ...
  - Test: `startTimer()` porta `status` a `running` e chiama `AlarmProvider.schedule`.
...
* **Dipendenze**: M2.T1
* **Criteri di accettazione**:
  - `TimerDisplay` renderizza `remainingSeconds` formattato `mm:ss`.
...
* **Criteri di accettazione**:
  - Le regole sono attive solo durante `status === 'running'` (fuori sessione focus, navigazione libera) — comportamento da confermare, vedi Punti da chiarire §4.
  - Test con `fakeBrowser`: cambiare la blocklist durante `running` triggera una chiamata a `updateDynamicRules` con le regole attese.
...
* **Obiettivo**: funzione pura `calculateScore(distractionRatio: number): 'excellent'|'good'|'bad'` secondo le soglie del documento (0% / ≤20% / ≥50%), più slice che ne mantiene lo stato per la sessione corrente.
* **File**: `lib/score/calculate-score.ts`, `store/score.slice.ts`
* **Dipendenze**: M1.T4
* **Criteri di accettazione**:
  - `calculateScore(0)` → `'excellent'`.
  - `calculateScore(0.2)` → `'good'`, `calculateScore(0.21)` → non `'good'` (comportamento nella fascia 20–50% da chiarire, vedi Punti da chiarire §4).
  - `calculateScore(0.5)` e oltre → `'bad'`.
  - Tutti i valori limite coperti da test parametrici.

#### M2.T10 — Calcolo malus su accesso a sito bloccato
* **Obiettivo**: quando arriva il messaggio `SITE_BLOCKED_ATTEMPT` (da M2.T8) con scelta "prosegui", il background calcola il malus e aggiorna lo stato città/score.
* **File**: `lib/score/apply-malus.ts`
* **Dipendenze**: M2.T9, M2.T8
* **Criteri di accettazione**:
  - Malus base fisso applicato per singolo evento di violazione (versione base, non proporzionale al tempo — quella è M4.T1).
  - Funzione pura: `applyMalus(cityState, event) → newCityState`, testabile senza browser.

#### M2.T11 — `citySlice` — stato griglia città
* **Obiettivo**: slice con `{ grid: CityGrid, buildings: Record<domain, BuildingMeta> }`.
* **File**: `store/city.slice.ts`
* **Dipendenze**: M1.T4
* **Criteri di accettazione**:
  - Stato iniziale è una griglia "vuota" di dimensioni configurabili.
  - Slice espone azioni `growCity(delta)`, `destroyBuilding(coords)` che delegano al growth engine (M2.T12), non calcolano internamente.

#### M2.T12 — `lib/city/growth-engine.ts`
* **Obiettivo**: motore puro che, dato lo stato griglia corrente e un "tempo di focus trascorso", calcola il nuovo stato (quali celle passano da `empty`→`growing`→`building`).
* **File**: `lib/city/growth-engine.ts`
* **Dipendenze**: M2.T11, M2.T1
* **Criteri di accettazione**:
  - Stesso stato + stesso delta di tempo → stesso risultato (determinismo, no `Date.now()` interno).
  - Test su almeno 3 step di crescita consecutivi che verificano la progressione di stato delle celle.
...
#### M3.T7 — Layout mobile-first popup
* **Obiettivo**: adeguare CSS/layout del popup per essere utilizzabile su viewport ridotti, coerente col requisito "mobile-first" del documento (nota: le web extension "mobile" hanno vincoli specifici — vedi Punti da chiarire §4).
* **File**: CSS/Tailwind o CSS Modules dei componenti coinvolti in M3.T4.
* **Dipendenze**: M3.T4
* **Criteri di accettazione**:
  - Il popup rimane utilizzabile (nessun overflow, controlli raggiungibili) a partire da una viewport di riferimento minima da concordare.
...
Le seguenti domande emergono da ambiguità o punti sottospecificati nel documento sorgente. Non sono state fatte assunzioni implicite su questi aspetti in fase di stesura dei task; le decisioni prese vanno confermate (o corrette) prima di iniziare l'implementazione dei task coinvolti.

1. **Ambito temporale del blocco siti** (rilevante per M2.T7): il blocco tramite `declarativeNetRequest` deve essere attivo *solo* durante una sessione di focus in corso (`status === 'running'`), oppure l'allowlist/blocklist deve poter restare attiva anche fuori sessione (es. come blocco "sempre attivo" configurabile)? Il documento descrive il blocco nel contesto del "periodo di focus" ma non chiarisce il comportamento fuori da esso.
2. **Fascia intermedia dello score** (rilevante per M2.T9): il documento definisce Excellent (0%), Good (≤20%) e Bad (≥50%), ma non specifica esplicitamente come classificare la fascia 20–50% (esclusi i due estremi già coperti). Va introdotto un quarto livello intermedio, oppure quella fascia va assorbita in "Good" o in "Bad"?
3. **Definizione di "tempo" per il calcolo della percentuale di distrazione**: la percentuale usata per lo score è calcolata sul tempo totale della sessione di focus pianificata, sul tempo effettivamente trascorso fino al momento della verifica, o su un'altra base? Necessario per implementare correttamente `calculateScore` in modo coerente con l'intento dell'utente.
4. **Capienza massima della griglia città** (rilevante per M2.T12): cosa succede quando la città raggiunge la capienza massima della griglia ma la sessione di focus continua (l'utente resta concentrato oltre il tempo "necessario" a riempire la città)? Il documento non specifica un comportamento di "città piena" (nuova griglia/livello, crescita in verticale, cap sullo score, ecc.).
5. **Persistenza dello storico sessioni**: il documento menziona un sistema di score per singola sessione, ma non chiarisce se debba esistere uno storico persistito delle sessioni passate (es. per statistiche nel tempo) o se lo score riguardi solo la sessione corrente, resettato ad ogni nuovo avvio. Questo impatta direttamente cosa va nello store persistito (Sezione 1.3) oltre a quanto già assunto.
6. **Comportamento del malus in caso di navigazione multi-tab**: se l'utente ha più tab aperte e una di queste accede a un sito bloccato mentre le altre restano su siti consentiti, il malus si applica una volta per tentativo o considera in qualche modo il tempo "netto" complessivo speso tra le tab? Rilevante per l'estensione futura M4.T1 (malus proporzionale al tempo).
7. **Requisito "mobile-first" per un'estensione browser** (rilevante per M3.T7): le estensioni browser tradizionali (Manifest V3, popup) non girano nativamente su browser mobili nello stesso modo dei desktop (il supporto extension varia tra Firefox Android e Chrome Android/iOS). Il requisito "mobile-first" si riferisce (a) al layout responsive del popup su schermi piccoli in generale, o (b) a un target esplicito di funzionamento su un browser mobile specifico? La risposta cambia sostanzialmente la portata di M3.T6/M3.T7.
8. **Granularità del malus "distruzione di edifici"**: un singolo tentativo di accesso a un sito bloccato distrugge un edificio specifico (quale? l'ultimo costruito? uno scelto casualmente?), un numero di edifici proporzionale alla gravità, o riduce genericamente un contatore di "popolazione" senza rimuovere edifici visibili in griglia? Il documento cita entrambi gli effetti ("perdita di popolazione e distruzione di edifici") senza specificarne la relazione esatta.