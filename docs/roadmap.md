...
* **Version control — l'agente non committa mai**: l'agente non esegue **mai** `git commit`, `git push`, `git tag`, `git merge`, `git rebase` o altri comandi che scrivono nella storia del repository (può usare git **solo in lettura**: `git status`/`log`/`diff`/`show`/`rev-parse`). Tutte le modifiche (codice, test, changelog) vanno lasciate **non committate**, pronte per la revisione e il commit a cura dello sviluppatore umano; nel changelog l'hash viene riportato solo per i commit già esistenti, altrimenti `—`/`da committare`.
...
* **Criteri di accettazione**:
  - `bun run dev` avvia WXT senza errori e produce un'estensione caricabile in Chrome.
